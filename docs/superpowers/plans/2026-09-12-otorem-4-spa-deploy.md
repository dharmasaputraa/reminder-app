...
**Goal:** A complete React frontend (Vite + TanStack Router/Query + Tailwind) embedded into the Go binary, plus Docker + compose packaging (cloudflared/gotify/litestream profiles) and an operational README.

**Architecture:** SPA same-origin at `/` served by Gin via `NoRoute` + `go:embed` (index.html fallback for client-side routing; 503 if not built yet). Build pipeline: `make web` copies `web/dist` → `internal/api/webroot/`; the multi-stage Dockerfile does the same. The API is used via `/api/v1` (dev auth: `X-Dev-Email` header from localStorage).

**Tech Stack:** Node ≥ 22, Vite 6, React 18, TypeScript, `@tanstack/react-router` (file-based + Vite plugin), `@tanstack/react-query` v5, Tailwind CSS v4 (`@tailwindcss/vite`).

## Global Constraints

- Consume endpoints exactly as in the Plan 3 contract (see the end of plan 3): `/upcoming`, `/contacts…`, `/channels…` (+ `POST /channels/:id/test`), `/settings`, `/pawukon?date=`, `/me`; dev header `X-Dev-Email`.
- Zero Node runtime in production — the static build output is embedded; `internal/api/webroot/*` is gitignored EXCEPT `.gitkeep`.
- UI in Indonesian; not v1 goals: i18n, dark mode, frontend unit tests (verification via `npm run build` + manual smoke).
- TanStack file-based routing: `routeTree.gen.ts` is generated by the plugin — never edit it by hand, it may be gitignored.
- Every task: build → verify (build/test) → commit.

---

### Task 1: Vite + TanStack + Tailwind scaffold

**Files:**
- Create: `web/` (scaffold output), `web/vite.config.ts`, `web/src/index.css`, `web/src/main.tsx`, `web/src/routes/__root.tsx`
- Modify: `web/index.html`

**Interfaces:**
- Produces: a running Vite app (`npm run dev` proxies `/api` → `:8080`); the file-based route tree is active; `QueryClient` is available via router context.
...
- [ ] **Step 1b: shadcn base for reUI components (event calendar, Task 3)**

shadcn needs the path alias + tsconfig before `init`:

`tsconfig.json` + `tsconfig.app.json` — add to `compilerOptions`:
...
`vite.config.ts` — resolve alias (merge with the Step 2 config):
...
```bash
npx shadcn@latest init -y -b neutral   # detects Vite + Tailwind v4; accept the defaults
```
Expected: `components.json` is created, `src/lib/utils.ts` (the `cn` helper) exists. If init overwrites `src/index.css` — restore it so `@import "tailwindcss";` is still there (the shadcn theme block below it is normal).

- [ ] **Step 2: Configure Vite (router plugin + tailwind + proxy + alias)**
...
`web/index.html` — make sure it contains (inside `<head>`, replacing the default title):
...
```
Expected: build succeeds, `dist/index.html` exists (no routes besides root yet — `/` renders the header).
...
```
(create-vite's default `web/.gitignore` already covers `node_modules`/`dist`.)

---

### Task 2: API client + placeholder route pages

**Files:**
- Create: `web/src/lib/api.ts`
- Create: `web/src/routes/index.tsx`, `web/src/routes/contacts.index.tsx`, `web/src/routes/contacts.$id.tsx`, `web/src/routes/channels.tsx`, `web/src/routes/settings.tsx` (minimal content in this task; later tasks finish them)

**Interfaces:**
- Produces: `api<T>(path, init?)` — fetch `/api/v1{path}`, inject `X-Dev-Email` from localStorage, prompt once on a dev 401; types `UpcomingItem`, `Contact`, `Occasion`, `Prefs`, `Channel`, `Settings`.
...
- [ ] **Step 2: Placeholder pages so the route tree is valid**
...
npm run build
```
Expected: success; `routeTree.gen.ts` is auto-generated and contains 5 routes.
...
### Task 3: Dashboard — upcoming timeline

**Files:**
- Modify: `web/src/routes/index.tsx` (replace the placeholder)
...
- [ ] **Step 1b: reUI event calendar on the Dashboard**

Component source: https://reui.io/components/event-calendar — free/open (MIT-style, code copied into the project); install via the shadcn CLI (requires Task 1 Step 1b):

```bash
npx shadcn@latest add @reui/c-event-calendar-3
```

(The CLI automatically installs the component's dependencies + the shadcn base components it needs. If the `c-event-calendar-3` variant is too heavy for read-only use, pick the simplest event-calendar variant that shows events in a month view + agenda — see the variant list on the source page.)

Integration in `routes/index.tsx`:
- Inspect the generated component for its `Event` type (usually `{ id, title, start, end, ... }` with `Date`), then map from `UpcomingItem`:
  `start = end = new Date(it.date + "T00:00:00")` (local midnight — avoid a UTC offset), `title = it.title` (the emoji is already included), keep `kind` for styling.
- Styling per kind: holidays vs occasions get a different variant/color (use the properties the component offers — if there is no per-event styling API, render the existing `🛕`/`📅` prefixes in `title` and let the defaults be).
- Default view: month; the Task 3 agenda/timeline list stays BELOW the calendar (the H-n badge + reminder chips have no calendar equivalent).
- Do NOT edit the generated reUI component internals; wrap it/props only. Store it in `src/components/event-calendar/` (CLI output).

- [ ] **Step 2: Visual verification (dev server + dev-mode API)**
...
cd web && npm run dev
```
Open `http://localhost:5173` → enter an email at the prompt → the Dashboard renders (it may be empty). Create a contact via `/contacts` (still a placeholder — create it via curl) and make sure the item appears:
...
### Task 4: Contacts — list, create, detail (occasions + prefs + pawukon preview)

**Files:**
- Modify: `web/src/routes/contacts.index.tsx`, `web/src/routes/contacts.$id.tsx`

- [ ] **Step 1: Contact list page**
...
- [ ] **Step 2: Detail page (occasions + prefs + pawukon preview)**
...
Note for the engineer: channel checkboxes use `defaultChecked` + mutate immediately on toggle (saved right away), while the offsets/enabled fields use the Save button. This is intentional — two save paths.
...
### Task 5: Channels — manage + test button
...
npm run build
```
Manually: add a fake gotify channel → press Tes → shows "GAGAL: …" (HTTP failure) — proof the test-send path is alive.
...
### Task 7: PWA + embed into the Go binary + NoRoute fallback
...
- Modify: `internal/api/server.go` (1 NoRoute route line)
- Create: `Makefile`
- Modify: `.gitignore`
- Test: `internal/api/spa_test.go`

**Interfaces:**
- Produces: `GET /` serves the embedded SPA; unknown paths → `index.html` (client routing); unknown `/api/*` → 404 JSON; not built yet → 503.
...
`web/public/sw.js` (minimal service worker — enough for installability, with no caching strategy so data stays fresh):
...
`web/src/main.tsx` — add at the end of the file (before the final `)` line):
...
`internal/api/webroot/.gitkeep` — an empty file (committed so `go:embed` does not fail before the first build).

Add to `internal/api/server.go` — right after the `r.GET("/metrics", ...)` line:
...
// spaHandler: serve the static files produced by `make web` (embedded). Unknown
// paths → index.html (client-side routing). Not built yet → 503 with a clear message.
...
func TestSPANotBuiltReturns503(t *testing.T) {
	...
	if w.Code != 503 { t.Errorf("/ without a webroot build: %d, want 503", w.Code) }
	w = httptest.NewRecorder()
	s.ServeHTTP(w, httptest.NewRequest("GET", "/api/v1/tidak-ada", nil))
	if w.Code != 404 { t.Errorf("unknown /api/*: %d, want 404", w.Code) }
}
```
...
.env
```
(note: `web/routeTree.gen.ts` is generated by the plugin — commit it for consistent builds; if it conflicts during a merge, regenerate it with `npm run build`.)

- [ ] **Step 4: End-to-end embed verification**
...
```
Expected: `/` → HTML containing `<div id="root">`; `/contacts/1` → 200 index.html (SPA fallback); `/api/v1/tidak-ada` → 404 JSON.
...
### Task 8: Docker + compose + README + final verification
...
# ---- 2: build the binary (embed the SPA) ----
...
```bash
# Required — channel config encryption key (AES-256-GCM). At least 16 characters.
APP_SECRET=ganti-dengan-string-acak-panjang

# Auth — fill in from Cloudflare Zero Trust (see README §Cloudflare Access)
AUTH_MODE=cfaccess
...
# Only for the cloudflared profile
TUNNEL_TOKEN=
```
...
```yaml
# SQLite replication to object storage — fill in per provider (S3/R2).
dbs:
  - path: /data/otorem.db
    replicas:
      - url: s3://bucket-anda/otorem/otorem.db
        # endpoint, access-key-id, secret-access-key via LITESTREAM_* env
```

- [ ] **Step 3: README.md**

README content (write it in full, required structure):
1. **Title + 2-sentence description** — Balinese otonan reminders (210-day Pawukon), birthdays, anniversaries, holidays; Gotify/Telegram/Email notifications; self-hosted in 1 container.
2. **Features** — bullet list from the spec (per-contact customizable offsets, dedupe + catch-up, holidays, PWA).
3. **Quickstart**:
   ```bash
   cp .env.example .env   # edit APP_SECRET, CF_ACCESS_*, ADMIN_EMAILS
   docker compose --profile cloudflared up -d --build
   ```
4. **Cloudflare Access setup**: Zero Trust → Access → Applications → Add self-hosted → domain `otorem.domain-anda.com` → email policy; note the **team domain** (`xxx.cloudflareaccess.com`) and **AUD** (Application Audience) → fill in `.env`; tunnel: Zero Trust → Networks → Tunnels → Create → copy TUNNEL_TOKEN.
5. **Dev mode** (without a tunnel): `AUTH_MODE=dev`, open the app, enter an email at the prompt; header `X-Dev-Email`.
6. **Notifications**: Gotify (create an app in the Gotify UI → copy the token), Telegram (@BotFather → token; chat_id via getUpdates), Email (Gmail app password or a transactional SMTP relay — sending from a home IP is spam-prone).
7. **Backup & restore**: the `./data` volume (SQLite WAL file) + the litestream option (`docker compose --profile litestream up -d`); restore: stop the app → `litestream restore -o /data/otorem.db s3://…` → start.
8. **Development**: `make test`, `make web`, `make run` (dev mode on :8080), fixture scraper `go run ./scripts/fetch_fixtures -year 2026` (data © kalenderbali.org — personal fixtures, do not redistribute).
9. **Project structure** — a short tree per spec §3.

- [ ] **Step 4: Final verification (everything must be green)**

```bash
CGO_ENABLED=0 go test ./... -count=1        # all tests green
...
curl -s localhost:8081/ | head -c 120        # SPA HTML from the embed
curl -s -H 'X-Dev-Email: a@b.c' -X POST localhost:8081/api/v1/scheduler/run
docker rm -f otorem-smoke
```
Expected: `{"ok":true}`; `<!doctype html>`/`<div id="root">` HTML; JSON counts from scheduler-run.
...
## Definition of Done (Plan 4 — and the otorem v1 project)

- [ ] `CGO_ENABLED=0 go test ./... -count=1` fully green.
- [ ] `make build` produces a single binary containing the SPA; `/` serves the UI; the deep link `/contacts/1` → 200.
- [ ] `docker compose config` is valid; `docker compose build app` succeeds; the smoke container passes healthz + SPA + scheduler-run.
- [ ] PWA installable from a phone (manifest + SW).
- [ ] The README covers Cloudflare Access setup, notifications, and backups.
- [ ] Tag `plan-4-spa-deploy-done`.

**Cross-plan execution note:** after the final tag, the recommended manual verification sequence (local device + tunnel): create a contact + an otonan with base = today−210 → `POST /scheduler/run` → the message arrives on the test channel (local Gotify) → stop the container for 1 hour → start it → run again → the missed reminder is delivered labeled "terlambat". Failing at any stage = go back to the relevant plan (1: dates, 2: API, 3: delivery, 4: packaging).
